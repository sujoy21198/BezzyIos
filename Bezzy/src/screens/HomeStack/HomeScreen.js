...
class HomeScreen extends React.Component {
  ...
  _handleAppStateChange = (nextAppState) => {
    ...
    this.setState(
      { expand: false, postDetails: [], appState: nextAppState },
      () => {
  ...
  fetchHomeListing = async (type) => {
    ...
    await axios
      .get(
        DataAccess.BaseUrl + DataAccess.friendBlockList + "/" + userId,
      ...
      .then(async (response) => {
        ...
        if (response.data.status === "success") {
          userList = [];
          response.data.total_feed_response.friend_list.map(
          ...
          if (this.state.activeSections.length > 0) {
            ...
            this.friendsBlockDetails(
              followingList[this.state.activeSections[0]].friend_id
            );
          }
        } else if (response.data.status === "error") {
          await axios
            .post(
              DataAccess.BaseUrl + DataAccess.userList,
              {
                log_userID: userId,
              },
              DataAccess.AuthenticationHeader
            ...
            .then(function (responseUserList) {
              if (responseUserList.data.resp === "success") {
                responseUserList.data.all_user_list =
                  responseUserList.data.all_user_list.filter(
    ...
    this.setState({
      userList,
      followingList: [...postUsers, ...noPostUsers],
      isRefreshing: false,
      isRefreshingFeed: false,
    });
    //this.RBSheet1.close();
        ...
        >
          <View style={{ flexDirection: "row" }}>
            ...
            >
              ...
              >
                {section.friend_name || "Anonymous User"}
              </Text>
              {section.today_post !== "" ? (
                <View>
                  <Text
                  ...
                  >
                    {Number(section.today_post) === 1
                      ? section.today_post + " New Post"
                      : section.today_post + " New Posts"}
                  </Text>
                </View>
              ) : (
                  ...
                  <View>
                    ...
                    >
                      ...
                      {Number(section.past_post_days) > 365
                        ? String(
                            parseInt(Number(section.past_post_days) / 365)
                          ) +
                          " " +
                          (Number(section.past_post_days) / 365 === 1
                            ? "year "
                            : "years ")
                        : Number(section.past_post_days) > 30
                        ? String(
                            parseInt(Number(section.past_post_days) / 30)
                          ) +
                          " " +
                          (Number(section.past_post_days) / 30 === 1
                            ? "month "
                            : "months ")
                        : String(section.past_post_days) +
                          " " +
                          (Number(section.past_post_days) === 1
                            ? "day "
                            : "days ")}
                      ago
                    </Text>
            ...
            {section.unread_post_number === "" ? (
              <View
                style={{
                  position: "absolute",
                  top: Platform.isPad ? heightToDp("0.5%") : 0,
                  right: widthToDp("1%"),
                }}
              >
                <Image
                  ...
                  style={{
                    height: Platform.isPad
                      ? heightToDp("4.5%")
                      : heightToDp("6%"),
                    width: Platform.isPad ? widthToDp("4.5%") : widthToDp("6%"),
                  }}
                />
              </View>
            ) : (
                  ...
                  routes: [
                    {
                      name: "InboxScreen",
                      params: {
                        friendId: section.friend_id,
                        friendImage: section.friend_photo,
                        friendName: section.friend_name,
                      },
                    },
                  ],
                })
      ...
      posts.map((i) => {
        if (i.post_id === item.post_id) {
          i.log_user_like_status === "No"
            ? (i.number_of_like += 1)
            : (i.number_of_like -= 1);
          i.log_user_like_status =
            i.log_user_like_status === "No" ? "Yes" : "No";
          i.isLiking = false;
        }
      });
      this.setState({ postDetails: posts });
        ...
        {this.state.isAccordianOpening ? (
          ...
          postDetails.length > 0 && (
            <FlatList
              ...
              onRefresh={() =>
                this.setState(
                  { isRefreshing: false, isRefreshingFeed: true },
                  () =>
                    this.friendsBlockDetails(
                      this.state.followingList[this.state.activeSections[0]]
                        .friend_id,
                      "pullRefresh"
                    )
                )
                          ...
                          renderItem={({ item, index }) => (
                            <TouchableOpacity
                              ...
                              onPress={() => {
                                for (
                                  let j = 0;
                                  j < i.item.post_img_video_live.length;
                                  j++
                                ) {
                                  if (this[`player${j}`])
                                    this[`player${j}`] = undefined;
                                }
                                this.setState({
                                  shouldPlay: false,
                                  postDetails: [],
                                ...
                                this.props.navigation.navigate(
                                  "ImagePreviewScreen",
                                  {
                                    image: { ...item, post_id: i.item.post_id },
                                  ...
                                  >
                                    ...
                                    >
                                      {index +
                                        1 +
                                        " / " +
                                        i.item.post_img_video_live.length}
                                    </Text>
                                  </View>
                                )}
                              </View>
                    ...
                    {i.item.post_type === "image" &&
                      i.item.post_img_video_live.length > 0 && (
                        <FlatList
                          ...
                          // }}
                          snapToInterval={Dimensions.get("window").width * 0.95}
                          decelerationRate="fast"
                          bounces={false}
                          getItemLayout={(item, index) => ({
                            length: Dimensions.get("window").width * 0.95,
                            offset:
                              Dimensions.get("window").width * 0.95 * index,
                            index,
                          })}
                          showsHorizontalScrollIndicator={true}
                    ...
                    >
                      ...
                      >
                        ...
                        <Icon2
                          ...
                          style={{ paddingLeft: widthToDp("4%") }}
                          onPress={() =>
                            this.setState({ sharepostID: i.item }, () =>
                              this.RBSheet2.open()
                            )
                          }
                        />
  ...
  friendsBlockDetails = async (id, type) => {
    if (type === "pullRefresh")
      this.setState({ isRefreshing: false, postDetails: [] });
    ...
    await axios
      .get(
        DataAccess.BaseUrl + DataAccess.friendblockdetails + id + "/" + userID,
        DataAccess.AuthenticationHeader
      )
      .then((response) => {
        if (response.data.status === "success") {
  ...
  shareImageInternally = async () => {
    ...
    console.log(
      DataAccess.BaseUrl +
        DataAccess.sharePostInternally +
        "/" +
        this.state.sharepostID.post_id +
        "/" +
        value +
        `/${this.state.sharepostID.post_type === "image" ? 1 : 3}`
    );
    await axios
      .get(
        DataAccess.BaseUrl +
  ...
  sendFollowRequest = async (item, index) => {
    this.RBSheet.open();
    let userId = await AsyncStorage.getItem("userId");
    await axios.post(
    ...
    return (
      <SafeAreaProvider style={{ flex: 1, backgroundColor: "#ececec" }}>
              ...
              <RefreshControl
                ...
                onRefresh={() =>
                  this.setState({ isRefreshing: true }, () =>
                    this.fetchHomeListing("pullRefresh")
                  )
                }
              />
            }
            ...
            <Accordion
              ...
              onChange={(activeSections) => {
                if (this.state.loadingPosts) {
                  let postsRequested = this.state.followingList.find(
                    (item) => item.expand
                  );
                  if (typeof postsRequested === "object") {
                    Toast.show({
                      text:
                        "Posts of " +
                        postsRequested.friend_name +
                        " are fetching. Please wait.",
                      style: {
                        backgroundColor: "#777",
                      },
                      duration: 1500,
            ...
            renderItem={({ item, index }) => (
              ...
              >
                ...
                <TouchableOpacity
                  ...
                  onPress={() => item.private_account === "0" ? this.followUser(item, index) : this.sendFollowRequest(item, index)}
                >
                  <Text
                    style={{
                      color: "#fff",
                      fontFamily: "ProximaNova-Black",
                      fontSize: item.private_account === "0" ? widthToDp("2%") : widthToDp("2.5%"),
                    }}
                  >
                    {item.user_is_flollowers === "No"
                      ? item.private_account === "0" ? "FOLLOW" : this.state.sendFollowRequest !== "" ? this.state.sendFollowRequest : "Send Follow Request"
                      : item.private_account === "0" ? "FOLLOW BACK" : this.state.sendFollowRequest !== "" ? this.state.sendFollowRequest : "Send Follow Back Request"}
                  </Text>
                </TouchableOpacity>
                <RBSheet
                  ref={(ref) => {
        ...
        >
          ...
          >
            <ShareIcon name={"share-apple"} size={Platform.isPad ? 50 : 25} />
            <Text
              style={{
                marginLeft: widthToDp("2%"),
                fontSize:
          ...
          >
            <ShareIcon name={"share-google"} size={Platform.isPad ? 50 : 25} />
            <Text
              style={{
                marginLeft: widthToDp("2%"),
                fontSize:
...
export default (props) => {
  ...
  return <HomeScreen {...props} insets={insets} />;
};
