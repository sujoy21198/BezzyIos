...
class InboxScreen extends Component {
  ...
  getIncomingMessage = async () => {
    ...
    await axios
      .get(
        DataAccess.BaseUrl +
          DataAccess.instantMessage +
          "/" +
          userId +
          "/" +
          this.props.route.params.friendId +
          "/1",
        DataAccess.AuthenticationHeader
      )
      .then((response) => {
        console.log(response.data, "getIncomingMessage");
        if (response.data.status === "success") {
          this.setState({ page: 1, message: [], oldMsgs: [] });
  ...
  getUserId = async () => {
    ...
    this.setState({isFetching: true}, () => this.getInboxChats("0"));
    // this.readChats()
  };
  ...
  getInboxChats = async (value) => {
    ...
    if (value === "0") {
      ...
      console.log(
        "API Called ==> ",
        DataAccess.BaseUrl +
          DataAccess.chatListInbox +
          this.state.userId +
          "/" +
          this.state.friendsId +
          "/" +
          this.state.page
      );
      await axios
  ...
  pagination = async () => {
    ...
    await axios
      ...
      .then((response) => {
        if (response.data.chat_history_list.length > 0) {
          response.data.chat_history_list.forEach((item) => {
            item.isSelected = false;
          });
          messages = response.data.chat_history_list;
          this.state.oldMsgs = response.data.chat_history_list;
          this.setState({ message: this.state.message.concat(messages) });
        } else {
          this.state.oldMsgs = [];
        }
      ...
      .catch((error) => {
        ...
        this.setState({ oldMsgs: [] });
        // Toast.show({
        //   type: "danger",
        //   text: "No more chat history is there.",
        //   duration: 3000,
        // });
      });
  ...
  sendMessage = async () => {
    ...
    await axios
      .post(
        DataAccess.BaseUrl + DataAccess.addChatData,
        {
  ...
  // }
  handleLoadMore = () => {
    if (this.state.oldMsgs.length === 0) return;
    this.setState(
      { page: this.state.page + 1, isFetchingMore: true },
      this.pagination
    );
  };
  ...
  postImageToChat = async () => {
    if (this.state.imagesArray.length <= 0) {
                  ...
                  PushNotification.localNotification({
                    ...
                    message:
                      ...
                      (this.state.imagesArray.length === 1
                        ? " image is sending. Please wait..."
                        : " images are sending. Please wait..."),
                  });
                });
              }
            );
      ...
      await axios
        .post(
          DataAccess.BaseUrl + DataAccess.addChatDataImage,
          formData,
          DataAccess.AuthenticationHeader
        )
        .then((response) => {
          ...
          Toast.show({
            ...
            text:
              ...
              (this.state.imagesArray.length === 1
                ...
                : " images have been sent successfully."),
            duration: 2000,
          });
          if (Platform.OS === "android") {
            PushNotification.getChannels((channelIds) => {
    ...
    console.log(
      ...
      DataAccess.BaseUrl +
        (this.state.deleteIds.length === 1
          ? DataAccess.deleteSingleMessage
          : DataAccess.deleteMultipleMessage)
    );
    Alert.alert(
      "Are you sure?",
      this.state.deleteIds.length +
        ` ${
          this.state.deleteIds.length === 1 ? "message" : "messages"
        } will be deleted`,
      [
        {
          text: "Cancel",
        },
        {
          ...
          onPress: async () => {
            ...
            await axios
              ...
              .then((response) => {
                console.log(
                  "Chat message delete success response :- ",
                  response.data,
  ...
  clearChat = async () => {
    ...
    console.log({
      from_user_id: userId,
      to_user_id: this.props.route.params.friendId,
    });
    await axios
      .post(
    ...
    return (
      ...
      >
          ...
          >
            ...
            <Text
              style={{
                ...
                fontSize: this.state.isSelected
                  ? widthToDp("4.5%")
                  : widthToDp("3.6%"),
          ...
          <KeyboardAvoidingView behavior="padding" style={{ flex: 1 }}>
                          ...
                          <TouchableOpacity
                            ...
                            onLongPress={() => {
                              ...
                              this.setState({
                                message: messages,
                                isSelected: true,
                                deleteIds: [...this.state.deleteIds, item.id],
                              });
                            }}
                            onPress={() => {
            ...
            <View
              style={{
                ...
                paddingHorizontal: widthToDp("1%"),
                paddingVertical: this.state.isKeyboardOpened
                  ? heightToDp("1%")
                  : heightToDp(Platform.OS === "android" ? "0.5%" : "0%"),
                flexDirection: "row",
                alignItems: "center",
              }}
            >
              <Icon
                ...
                style={{
                  paddingTop: this.state.isKeyboardOpened
                    ? heightToDp("0.5%")
                    : heightToDp("0%"),
                }}
              />
                      ...
                      {item.message_by === "self" && item.type === "text" ? (
                        ...
                        >
                          ...
                          >
                            ...
                            />
                            {/* <Text style={{ marginLeft: widthToDp("2%"), color: 'white', fontSize: widthToDp("3.6%"), fontFamily: "Poppins-Regular" }}>{item.chat_message}</Text> */}
                            <Text
                              style={{
                                marginRight: widthToDp("2%"),
                                color: "white",
                        ...
                        >
                          ...
                          >
                            ...
                            />
                            {/* <Text style={{ marginLeft: widthToDp("2%"), fontSize: widthToDp("3.6%"), fontFamily: "Poppins-Regular" }}>{item.chat_message}</Text> */}
                            <Text
                              style={{
                                marginRight: widthToDp("2%"),
                                color: "black",
        ...
        <EmojiBoard
          ...
          onClick={(value) => this.endEmoji(value)}
        />
...
export default (props) => {
  ...
  return <InboxScreen {...props} insets={insets} />;
};
