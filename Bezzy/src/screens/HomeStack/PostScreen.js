    ...
    if (this.state.focusedTab === "photo") {
      if (this.state.fromCamera === false) {
        if (this.state.imagesArray.length <= 0) {
          ...
          await axios
            .post(
              "https://bezzy-app.com/admin/api/PostImage",
              formData,
            ...
            .then(async (response) => {
              ...
              if (this.state.tagUserId.length > 0) {
                await axios
                  .post(
                    DataAccess.BaseUrl + DataAccess.postTaggedUserContent,
                    {
                      post_id: response.data.post_id,
                      log_user_id: userID,
                      post_content: this.state.caption,
                      post_type_id: "1",
                      tag_user_id: JSON.stringify(this.state.tagUserId)
                        .split("[")[1]
                        .split("]")[0],
                    },
                    DataAccess.AuthenticationHeader
              ...
              if (Platform.OS === "android") {
                ...
                PushNotification.localNotification({
                  title: "Image Upload",
                  channelId:
                    Platform.OS === "android" ? undefined : "imageupload",
                  id: Platform.OS === "android" ? undefined : "imageupload",
                  message: "Your image has been uploaded successfully.",
                });
              }
      ...
      if (this.state.thumbnail === "") {
        ...
        await axios
          .post(
            "https://bezzy-app.com/admin/api/PostVideo",
            fromDataVideo,
            DataAccess.AuthenticationHeader
          )
          .then(async (response) => {
            ...
            if (this.state.tagUserId.length > 0) {
              await axios
                .post(
                  DataAccess.BaseUrl + DataAccess.videoPostTaggedUserContent,
                  {
                    post_id: response.data.post_id,
                    log_user_id: userID,
                    post_content: this.state.caption,
            ...
            if (Platform.OS === "android") {
              ...
              PushNotification.localNotification({
                title: "Video Upload",
                channelId:
                  Platform.OS === "android" ? undefined : "videoupload",
                id: Platform.OS === "android" ? undefined : "videoupload",
                message: "Your video has been uploaded successfully.",
              });
            }
    ...
    return (
      <SafeAreaProvider style={{ flex: 1 }}>
        ...
        <Header
          ...
          headerText={this.state.focusedTab === "photo" ? "Photo" : "Video"}
        />
        ...
        >
            ...
            onChangeText={(text) => {
              this.setState({ caption: text, followingList: [] }, () => {
                if (
                  ...
                  this.state.caption.split(" ").length > 0
                ) {
                  // console.warn(this.state.caption.split(" "));
                }
                if (
                  this.state.caption.includes("@") &&
                  this.state.caption.match(/\B@\w+/g) &&
                  this.state.caption.match(/\B@\w+/g).length > 0 &&
                  this.state.caption.split(" ") &&
                  this.state.caption.split(" ").length > 0 &&
                  this.state.caption.split(" ")[
                    this.state.caption.split(" ").length - 1
                  ] !== "" &&
                  this.state.caption.split(" ")[
                    this.state.caption.split(" ").length - 1
                  ] !== "@" &&
                  this.state.caption
                    .split(" ")
                    [this.state.caption.split(" ").length - 1].includes("@")
                ) {
                  this.getFollowings(
                    this.state.caption.match(/\B@\w+/g)[
                      this.state.caption.match(/\B@\w+/g).length - 1
                    ]
                  );
          ...
          />
          <View
            style={{ alignItems: "flex-end", paddingBottom: heightToDp("1%") }}
          >
            <View
              style={{
                flexDirection: "row",
              ...
              >
                ...
                >
                  {this.state.focusedTab === "photo"
                    ? (this.state.buttonText = "UPLOAD PHOTO")
                    : (this.state.buttonText = "UPLOAD VIDEO")}
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={{
        ...
        {this.state.focusedTab === "photo" && this.state.fromCamera === true ? (
          ...
          this.state.fromCamera === false ? (
          <FlatList
            data={imagesArray}
            numColumns={2}
            ListFooterComponent={<View style={{ height: heightToDp("10%") }} />}
            ItemSeparatorComponent={() => (
              <View style={{ width: widthToDp("0.5%") }} />
            )}
            ...
            >
              ...
              >
                <Icon
                  ...
                  color={
                    this.state.focusedTab === "photo" ? "#007dfe" : "#808080"
                  }
                />
                <Text
                  style={{
              ...
              >
                <Icon
                  ...
                  size={Platform.isPad ? 40 : 23}
                  color={
                    this.state.focusedTab === "video" ? "#007dfe" : "#808080"
                  }
                />
                <Text
                  style={{
              ...
              <TouchableOpacity
                onPress={() => {
                  this.setState(
                    {
                      caption:
                        this.state.caption.substring(
                          0,
                          this.state.caption.trim().length -
                            this.state.caption.match(/\B@\w+/g)[
                              this.state.caption.match(/\B@\w+/g).length - 1
                            ].length
                        ) +
...
export default (props) => {
  ...
  return <PostScreen {...props} insets={insets} />;
};
