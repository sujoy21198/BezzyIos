...
class OtpVerify extends React.Component {
  ...
  otpVerify = async () => {
    ...
    if (
      ...
      let response = await axios.post(
        DataAccess.BaseUrl + DataAccess.SendOtp,
        {
      ...
      if (response.data.resp === "true") {
        this.RBSheet.close();
        if (
          this.props.route.params.type === "verify" ||
          this.props.route.params.type === "loginVerify"
        ) {
          Toast.show({
            text:
              this.props.route.params.type === "verify"
                ? "Your account is registered & activated successfully. Now you can login to the app."
  ...
  resendOtp = async () => {
    ...
    this.RBSheet.open();
    let response = await axios.post(
      DataAccess.BaseUrl + DataAccess.resendOtp,
      {
        userID: this.props.route.params.userId,
      },
      DataAccess.AuthenticationHeader
    ...
    <SafeAreaProvider style={{ flex: 1, backgroundColor: "#69abff" }}>
      ...
      >
        ...
        >
          ...
          <TouchableOpacity activeOpacity={0.7} onPress={this.resendOtp}>
            <Text
              style={{
                marginTop: heightToDp("2%"),
                fontSize: widthToDp("3.8%"),
...
export default (props) => {
  ...
  return <OtpVerify {...props} insets={insets} />;
};
